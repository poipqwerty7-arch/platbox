...
const routes = {
  ...
  'GET /api/games': async (req, res) => {
    const { rows: games } = await pool.query(`
      SELECT games.id, games.title, games.description, games.author_id, games.thumbnail_color, games.play_count, games.created_at, users.username as author_name
      FROM games LEFT JOIN users ON games.author_id = users.id
      ORDER BY games.play_count DESC
    `);
    return sendJSON(res, 200, { games });
  },

  'GET /api/my-games': async (req, res) => {
    const user = await getUserFromAuth(req);
    if (!user) return sendJSON(res, 401, { error: 'Нужно войти' });
    const { rows: games } = await pool.query(
      `SELECT id, title, thumbnail_color, play_count, created_at
       FROM games WHERE author_id = $1 ORDER BY created_at DESC`,
      [user.id]
    );
    return sendJSON(res, 200, { games });
  },

  'GET /api/games/:id': async (req, res, params) => {
    const id = Number(params.id);
    if (!Number.isInteger(id)) return sendJSON(res, 400, { error: 'Некорректный id игры' });
  ...
  'POST /api/games': async (req, res) => {
    const user = await getUserFromAuth(req);
    if (!user) return sendJSON(res, 401, { error: 'Нужно войти, чтобы публиковать игры' });

    const { title, level_data, thumbnail_color } = await readBody(req);
    const cleanTitle = typeof title === 'string' ? title.trim().slice(0, 80) : '';
    if (!cleanTitle) return sendJSON(res, 400, { error: 'Укажи название игры' });
    if (typeof level_data !== 'object' || level_data === null) {
      return sendJSON(res, 400, { error: 'Некорректные данные уровня' });
    }

    const colors = ['#e74c3c', '#3498db', '#2ecc71', '#f39c12', '#9b59b6', '#1abc9c'];
    const thumb = (typeof thumbnail_color === 'string' && /^#[0-9a-fA-F]{6}$/.test(thumbnail_color))
      ? thumbnail_color
      : colors[Math.floor(Math.random() * colors.length)];

    const { rows } = await pool.query(
      `INSERT INTO games (title, author_id, thumbnail_color, level_data)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [cleanTitle, user.id, thumb, JSON.stringify(level_data)]
    );
    return sendJSON(res, 201, { game: rows[0] });
  },

  'PUT /api/games/:id': async (req, res, params) => {
    const user = await getUserFromAuth(req);
    if (!user) return sendJSON(res, 401, { error: 'Нужно войти' });
    ...
    const { rows: existingRows } = await pool.query('SELECT author_id FROM games WHERE id = $1', [id]);
    if (!existingRows[0]) return sendJSON(res, 404, { error: 'Игра не найдена' });
    if (existingRows[0].author_id !== user.id) {
      return sendJSON(res, 403, { error: 'Редактировать игру может только её автор' });
    }
    ...
    if (typeof title === 'string' && title.trim()) {
      ...
      values.push(title.trim().slice(0, 80));
    }
    if (level_data && typeof level_data === 'object') {
      updates.push(`level_data = $${i++}`);
      values.push(JSON.stringify(level_data));
    }
    if (typeof thumbnail_color === 'string' && /^#[0-9a-fA-F]{6}$/.test(thumbnail_color)) {
      updates.push(`thumbnail_color = $${i++}`);
      values.push(thumbnail_color);
    }
    if (updates.length === 0) return sendJSON(res, 400, { error: 'Нечего обновлять' });

    values.push(id);
    const { rows } = await pool.query(
      `UPDATE games SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    );
    return sendJSON(res, 200, { game: rows[0] });
  },
