...
func _ready() -> void:
	...
	# Если с прошлого запуска остался сохранённый токен — проверяем его на
	# сервере и, если он ещё действителен, пускаем сразу в лобби без формы.
	if Session.token != "":
		login_button.disabled = true
		register_button.disabled = true
		subtitle.text = "Проверяем сохранённый вход…"
		Session.verify_session(func(success: bool):
			if success:
