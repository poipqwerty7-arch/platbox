...
# {"type": "test_return", "level_data": {...}} — возврат из Test Play (см. ниже).
var editor_load_request = null

# Test Play: игрок тестирует свой уровень прямо из редактора, без публикации.
# is_test_play — флаг для world.gd: "Назад" должен вернуть в редактор, а не в лобби.
# editor_return_data — снимок уровня (включая несохранённые изменения), чтобы
# редактор открылся именно с тем состоянием, из которого начали тест.
var is_test_play: bool = false
var editor_return_data = null
