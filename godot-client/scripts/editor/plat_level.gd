...
## Прямые дети узла parent_id в дереве Workspace ("" — объекты верхнего уровня).
func get_children(parent_id: String) -> Array[PlatObject]:
	var result: Array[PlatObject] = []
	for obj in workspace_objects:
		if obj.parent_id == parent_id:
			result.append(obj)
	return result

## Точка спавна: если на уровне есть объект-маркер Spawn Point — используем
## его позицию (первый найденный), иначе — старое поле spawn_point (для
## обратной совместимости со старыми сохранёнными уровнями без маркера).
func get_spawn_position() -> Vector3:
	for obj in workspace_objects:
		if obj.shape == PlatObject.Shape.SPAWN_POINT:
			return obj.position
	return spawn_point
