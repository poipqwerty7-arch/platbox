...
enum Shape { CUBE, CYLINDER, SPHERE, WEDGE }
...
func _build_wedge_mesh() -> ArrayMesh:
	...
	# Простая треугольная призма (клин), нормали пересчитываются автоматически.
	var faces := [
		[0, 1, 4], [1, 5, 4],          # передний скос
		[2, 4, 0], [2, 5, 4], [2, 5, 3], [5, 1, 3],  # боковины/низ упрощённо
	]
	for face in faces:
		for idx in face:
