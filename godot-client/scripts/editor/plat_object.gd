...
enum Shape { CUBE, CYLINDER, SPHERE, WEDGE, NPC, SPAWN_POINT, SCRIPT, LOCAL_SCRIPT }

# У Script/LocalScript нет 3D-геометрии — они существуют только как узлы
# в дереве Workspace (задел на будущий полноценный скриптинг).
const NON_VISUAL_SHAPES := [Shape.SCRIPT, Shape.LOCAL_SCRIPT]
...
var parent_id: String = ""          # id родителя в Workspace; "" = верхний уровень
...
func spawn_into(parent: Node3D) -> Node3D:
	if shape in NON_VISUAL_SHAPES:
		# Script/LocalScript живут только в дереве Workspace, в 3D-мире их нет.
		return null
	...
	var force_color: Variant = null  # для типов со своим фиксированным цветом (Spawn Point)
	...
	match shape:
		...
		Shape.SPAWN_POINT:
			# Плоский диск-маркер точки спавна — всегда ярко-зелёный и полупрозрачный,
			# чтобы явно отличаться от обычных блоков независимо от цвета в инспекторе.
			...
			disc_shape.radius = 0.9
			disc_shape.height = 0.15
			collision.shape = disc_shape
			force_color = Color(0.15, 0.95, 0.35, 0.75)
			body.set_meta("is_spawn_point", true)

	mesh_instance.scale = scale
	collision.scale = scale

	var mat := StandardMaterial3D.new()
	mat.albedo_color = force_color if force_color != null else color
	if force_color != null:
	...
	if shape == Shape.SPAWN_POINT:
		...
		marker_mat.albedo_color = Color(0.1, 1.0, 0.3, 0.9)
