...
signal draft_save_requested(title: String)
...
signal add_object_requested(shape: int, parent_id: String)
signal rename_requested(obj_id: String, new_name: String)
...
@onready var workspace_menu: PopupMenu = $WorkspaceContextMenu
@onready var workspace_add_submenu: PopupMenu = $WorkspaceContextMenu/AddSubmenu
@onready var rename_dialog: Window = $RenameDialog
@onready var rename_input: LineEdit = $RenameDialog/Margin/VBox/RenameInput
@onready var rename_confirm_button: Button = $RenameDialog/Margin/VBox/ConfirmButton
...
@onready var draft_button: Button = $SaveDialog/Margin/VBox/DraftButton
...
func _ready() -> void:
	...
	$Toolbar/TestPlayButton.pressed.connect(func(): test_play_requested.emit())
	...
	draft_button.pressed.connect(func():
		draft_save_requested.emit(title_input.text if title_input.text != "" else "Без названия")
	)
	...
	_setup_context_menu()
	_setup_workspace_context_menu()
	rename_confirm_button.pressed.connect(_on_rename_confirmed)
	workspace_tree.item_selected.connect(_on_workspace_item_selected)
	workspace_tree.item_mouse_selected.connect(_on_workspace_item_mouse_selected)
	workspace_tree.empty_clicked.connect(_on_workspace_empty_clicked)
...
## --- Контекстное меню (правый клик по объекту в 3D-сцене) ---
...
## --- Контекстное меню панели Workspace (правый клик по объекту/пустому месту) ---

const ADD_MENU_ITEMS := [
	["Блок", PlatObject.Shape.CUBE],
	["Цилиндр", PlatObject.Shape.CYLINDER],
	["Сфера", PlatObject.Shape.SPHERE],
	["Клин", PlatObject.Shape.WEDGE],
	["NPC", PlatObject.Shape.NPC],
	["Точка спавна", PlatObject.Shape.SPAWN_POINT],
	["Script", PlatObject.Shape.SCRIPT],
	["LocalScript", PlatObject.Shape.LOCAL_SCRIPT],
]

# "" — добавление объекта верхнего уровня (клик по пустому месту в Workspace).
# Иначе — id объекта, внутрь которого добавляем/над которым выполняем действие.
var _workspace_context_target_id: String = ""
var _rename_target_id: String = ""

func _setup_workspace_context_menu() -> void:
	workspace_add_submenu.clear()
	for entry in ADD_MENU_ITEMS:
		workspace_add_submenu.add_item(entry[0], entry[1])
	workspace_add_submenu.id_pressed.connect(func(shape_id: int):
		add_object_requested.emit(shape_id, _workspace_context_target_id)
	)

	workspace_menu.clear()
	workspace_menu.add_submenu_item("➕ Добавить", "AddSubmenu")
	workspace_menu.add_item("⧉ Дублировать", 100)
	workspace_menu.add_item("🗑 Удалить", 101)
	workspace_menu.add_item("✏ Переименовать", 102)
	workspace_menu.id_pressed.connect(func(id: int):
		match id:
			100: duplicate_requested.emit()
			101: delete_requested.emit()
			102: _open_rename_dialog(_workspace_context_target_id)
	)

func _on_workspace_item_mouse_selected(position: Vector2, mouse_button_index: int) -> void:
	if mouse_button_index != MOUSE_BUTTON_RIGHT:
		return
	var item := workspace_tree.get_item_at_position(position)
	if not item:
		return
	# Правый клик по объекту сразу его выбирает — чтобы Дублировать/Удалить/
	# Переименовать применялись именно к нему, а не к тому, что было выбрано раньше.
	# ВАЖНО: сначала читаем metadata, потом select() — select() триггерит
	# полную перестройку дерева (через сигнал), после которой этот TreeItem
	# уже невалиден.
	_workspace_context_target_id = item.get_metadata(0)
	item.select(0)
	_set_workspace_menu_has_target(true)
	workspace_menu.position = Vector2i(get_global_mouse_position())
	workspace_menu.popup()

func _on_workspace_empty_clicked(position: Vector2, mouse_button_index: int) -> void:
	if mouse_button_index != MOUSE_BUTTON_RIGHT:
		return
	_workspace_context_target_id = ""
	_set_workspace_menu_has_target(false)
	workspace_menu.position = Vector2i(get_global_mouse_position())
	workspace_menu.popup()

func _set_workspace_menu_has_target(has_target: bool) -> void:
	# Индексы: 0 = Добавить (submenu, доступен всегда), 1 = Дублировать, 2 = Удалить, 3 = Переименовать.
	workspace_menu.set_item_disabled(1, not has_target)
	workspace_menu.set_item_disabled(2, not has_target)
	workspace_menu.set_item_disabled(3, not has_target)

func _open_rename_dialog(obj_id: String) -> void:
	if obj_id == "":
		return
	_rename_target_id = obj_id
	var obj: PlatObject = editor_ref.level.find_object(obj_id)
	rename_input.text = obj.name if obj else ""
	rename_dialog.visible = true
	rename_input.grab_focus()

func _on_rename_confirmed() -> void:
	if _rename_target_id != "" and rename_input.text.strip_edges() != "":
		rename_requested.emit(_rename_target_id, rename_input.text.strip_edges())
	rename_dialog.visible = false

## --- Панель Workspace (список объектов, как в Roblox Studio) ---

var _refreshing_workspace := false  # guard от реентрантного вызова item_selected
...
func refresh_workspace(level: PlatLevel, selected_id: String) -> void:
	...
	workspace_tree.item_selected.disconnect(_on_workspace_item_selected)
	workspace_tree.clear()
	var root := workspace_tree.create_item()
	workspace_tree.hide_root = true
	_build_workspace_tree_level(level, root, "", selected_id)
	workspace_tree.item_selected.connect(_on_workspace_item_selected)

	_refreshing_workspace = false

## Рекурсивно строит дерево Workspace — как Model/Folder в Roblox Studio,
## объекты могут быть вложены друг в друга произвольно глубоко.
func _build_workspace_tree_level(level: PlatLevel, parent_item: TreeItem, parent_id: String, selected_id: String) -> void:
	for obj in level.get_children(parent_id):
		var item := workspace_tree.create_item(parent_item)
		item.set_text(0, "%s %s" % [_icon_for_shape(obj.shape), obj.name])
		item.set_metadata(0, obj.id)
		if obj.id == selected_id:
			item.select(0)
		_build_workspace_tree_level(level, item, obj.id, selected_id)

func _icon_for_shape(shape: int) -> String:
	match shape:
		PlatObject.Shape.NPC: return "🧍"
		PlatObject.Shape.SPAWN_POINT: return "🟢"
		PlatObject.Shape.SCRIPT: return "📜"
		PlatObject.Shape.LOCAL_SCRIPT: return "📄"
		_: return "◻"
...
func is_mouse_over_ui() -> bool:
	...
	for panel in [inspector_panel, $Toolbar, $BottomBar, $WorkspacePanel]:
...
func show_inspector(obj: PlatObject) -> void:
	if obj == null:
		push_warning("show_inspector вызван с null — пропускаем, чтобы не падать")
		hide_inspector()
		return
