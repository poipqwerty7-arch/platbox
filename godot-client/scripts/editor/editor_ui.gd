...
var _refreshing_workspace := false  # guard от реентрантного вызова (см. комментарий ниже)

func refresh_workspace(level: PlatLevel, selected_id: String) -> void:
	# ВАЖНО: программный item.select(0) ниже сам генерирует сигнал item_selected.
	# disconnect/connect ниже разрывает большинство случаев, но сигналы Tree в Godot
	# иногда доставляются отложенно (на следующий кадр), так что одного disconnect
	# недостаточно — добавляем ещё и явный guard-флаг, чтобы повторный вход в эту
	# функцию гарантированно не ломал недостроенное дерево (root/item = null).
